...
class Reward {
 public:
  ...
  const std::map<std::string, float>& getStdMap_l() {
    for(auto& rw: rewards_)
      rewardMap_l_[rw.first] = rw.second.reward;
    rewardMap_l_["reward_sum_l"] = sum();

    return rewardMap_l_;
  }
  const std::map<std::string, float>& getStdMap_r() {
    for(auto& rw: rewards_)
      rewardMap_r_[rw.first] = rw.second.reward;
    rewardMap_r_["reward_sum_r"] = sum();
 ...
 private:
  ...
  std::map<std::string, float> rewardMap_l_;
  std::map<std::string, float> rewardMap_r_;
};
