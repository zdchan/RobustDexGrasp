...
class RolloutStorage:
    def __init__(self, num_envs, num_transitions_per_env, actor_obs_shape, critic_obs_shape, actions_shape, total_student_obs_dim, latent_dim, device):
        self.device = device
        ...
        # self.expert_obs = np.zeros([num_transitions_per_env, num_envs, *actor_obs_shape], dtype=np.float32)
        # self.expert_actions = np.zeros([num_transitions_per_env, num_envs, *actions_shape], dtype=np.float32)
        # self.expert_latent = np.zeros([num_transitions_per_env, num_envs, latent_dim], dtype=np.float32)
        self.student_total_obs = np.zeros([num_transitions_per_env, num_envs, total_student_obs_dim], dtype=np.float32)
        ...
        # self.expert_obs_tc = torch.from_numpy(self.expert_obs).to(self.device)
        # self.expert_actions_tc = torch.from_numpy(self.expert_actions).to(self.device)
        # self.expert_latent_tc = torch.from_numpy(self.expert_latent).to(self.device)
        self.student_total_obs_tc = torch.from_numpy(self.student_total_obs).to(self.device)
    ...
    def add_transitions(self, actor_obs, critic_obs, actions, mu, sigma, rewards, dones, actions_log_prob, student_total_obs):
        ...
        # self.expert_obs[self.step] = student_total_obs[:, -actor_obs.shape[1]:]
        # self.expert_actions[self.step] = expert_actions
        # self.expert_latent[self.step] = expert_latent
        self.student_total_obs[self.step] = student_total_obs
    ...
    def mini_batch_generator_shuffle(self, num_mini_batches):
        ...
        for indices in BatchSampler(SubsetRandomSampler(range(batch_size)), mini_batch_size, drop_last=True):
            ...
            # expert_obs_batch = self.expert_obs_tc.view(-1, *self.expert_obs_tc.size()[2:])[indices]
            # expert_actions_batch = self.expert_actions_tc.view(-1, self.expert_actions_tc.size(-1))[indices]
            # expert_latent_batch = self.expert_latent_tc.view(-1, self.expert_latent_tc.size(-1))[indices]
            student_total_obs_batch = self.student_total_obs_tc.view(-1, *self.student_total_obs_tc.size()[2:])[indices]
            yield actor_obs_batch, critic_obs_batch, actions_batch, sigma_batch, mu_batch, values_batch, advantages_batch, returns_batch, old_actions_log_prob_batch, student_total_obs_batch
    ...
    def mini_batch_generator_inorder(self, num_mini_batches):
        ...
        for batch_id in range(num_mini_batches):
            yield self.actor_obs_tc.view(-1, *self.actor_obs_tc.size()[2:])[batch_id*mini_batch_size:(batch_id+1)*mini_batch_size], \
                ...
                self.student_total_obs_tc.view(-1, *self.student_total_obs_tc.size()[2:])[batch_id*mini_batch_size:(batch_id+1)*mini_batch_size]
                # self.expert_obs_tc.view(-1, *self.expert_obs_tc.size()[2:])[batch_id*mini_batch_size:(batch_id+1)*mini_batch_size], \
                # self.expert_actions_tc.view(-1, self.expert_actions_tc.size(-1))[batch_id*mini_batch_size:(batch_id+1)*mini_batch_size], \
                # self.expert_latent_tc.view(-1, self.expert_latent_tc.size(-1))[batch_id*mini_batch_size:(batch_id+1)*mini_batch_size], \
