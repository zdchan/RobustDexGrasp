...
inline void quatInvQuatMul( const Vec<4>&  q,  const Vec<4>&  p, Vec<4>& pq) {
  pq[0] = p[0] * q[0] - p[1] * -q[1] - p[2] * -q[2] - p[3] * -q[3];
  pq[1] = p[0] * -q[1] + p[1] * q[0] - p[2] * -q[3] + p[3] * -q[2];
  pq[2] = p[0] * -q[2] + p[1] * -q[3] + p[2] * q[0] - p[3] * -q[1];
  pq[3] = p[0] * -q[3] - p[1] * -q[2] + p[2] * -q[1] + p[3] * q[0];
...
inline void eulerToQuat(const Vec<3>& eulerVec, Vec<4>& quat) {
        double ai = eulerVec[2]/2.0;
        double aj = -eulerVec[1]/2.0;

        double ak = eulerVec[0]/2.0;
...
inline void quatToEuler(const Vec<4>& quat, Vec<3>& eulerVec) {
    const double w2 = quat[0]*quat[0];
    const double x2 = quat[1]*quat[1];
    const double y2 = quat[2]*quat[2];
    const double z2 = quat[3]*quat[3];
    const double unitLength = w2 + x2 + y2 + z2;    // Normalised == 1, otherwise correction divisor.
    const double abcd = quat[0]*quat[1] + quat[2]*quat[3];
    const double eps = 8.881784197001252e-16;
    const double pi = 3.14159265358979323846;
    if (abcd > (0.5-eps)*unitLength)
    {
        eulerVec[2] = 2 * atan2(quat[2], quat[0]);
        eulerVec[1] = pi;
        eulerVec[0] = 0;
    }
    ...
    {
        const double adbc = quat[0]*quat[3] - quat[1]*quat[2];
        const double acbd = quat[0]*quat[2] - quat[1]*quat[3];
        eulerVec[2] = ::atan2(2*adbc, 1 - 2*(z2+x2));
        eulerVec[1] = ::asin(2*abcd/unitLength);
        eulerVec[0] = ::atan2(2*acbd, 1 - 2*(y2+x2));
    }
}
...
inline void RotmatToEuler(Mat<3, 3> &mat1, Vec<3>& eulerVec) {
    double cy = std::sqrt(mat1.row(2)[2]*mat1.row(2)[2] + mat1.row(1)[2]*mat1.row(1)[2]);
    double eps = 8.881784197001252e-16;

    if (cy > eps){
        eulerVec[0] = -std::atan2(mat1.row(1)[2],mat1.row(2)[2]);
        eulerVec[1] = -std::atan2(-mat1.row(0)[2],cy);
        eulerVec[2] = -std::atan2(mat1.row(0)[1],mat1.row(0)[0]);
    }

    else{
        eulerVec[0] = 0.0;
        eulerVec[1] = -std::atan2(-mat1.row(0)[2],cy);
        eulerVec[2] =  -std::atan2(-mat1.row(1)[0],mat1.row(1)[1]);
    }
